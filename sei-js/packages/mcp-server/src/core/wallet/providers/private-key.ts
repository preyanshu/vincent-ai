...
export class PrivateKeyWalletProvider implements WalletProvider {
	...
	async getAddress(privateKey?: string): Promise<Address> {
		// Use provided privateKey or fall back to instance privateKey
	...
	async getWalletClient(network: string, privateKey?: string): Promise<WalletClient> {
		// Use provided privateKey or fall back to instance privateKey
		const keyToUse = privateKey || this.privateKey;
