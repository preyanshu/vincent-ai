 ...
 */
export async function writeContract(params: Record<string, any>, privateKey: string, network = DEFAULT_NETWORK): Promise<Hash> {
	const client = await getWalletClientFromProvider(network, privateKey);
